...
|---|---|---|---|
| 1 | [nullbyte-pentest](projects/nullbyte-pentest) | Full exploitation chain: recon → steganography → brute force → SQLi → hash cracking → root privilege escalation | Nmap, Nikto, Gobuster, Hydra, sqlmap, SUID/PATH hijacking |
| 2 | [MobSF-Apk-Testing](projects/MobSF-Apk-Testing) | Static/dynamic analysis of Android APKs — insecure storage, hardcoded secrets, permission misuse | MobSF |
| 3 | [Owasp-DVWA-bWAPP-Mutillde-Pentesting](projects/Owasp-DVWA-bWAPP-Mutillde-Pentesting) | Vulnerability assessment across intentionally vulnerable apps — SQLi, XSS, broken authentication | bWAPP, DVWA, Mutillidae |
| 4 | [tryhackme-web-security-labs](projects/tryhackme-web-security-labs) | Structured, mentor-guided labs on OWASP Top 10 exploitation techniques | TryHackMe |
| 5 | [wordpress-vulnerability-testing](projects/wordpress-vulnerability-testing) | End-to-end exploitation: payload delivery, reverse shell via Netcat, credential extraction, hash cracking with Hashcat | Netcat, Hashcat, Python HTTP server |
